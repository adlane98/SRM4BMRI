...
def downscale_3d_image(img, scale_factor):
    ...
    for i in range(z):
        gt[0,i,:,:] = img[:,:,i]
        inp[0,i,:,:] = cv.resize(img[:,:,i],(img.shape[0]//scale_factor,img.shape[1]//scale_factor))
    return gt, inp
...
downscaled_image = upscale(test_images[0], model_path)
tf.reset_default_graph()
...
plt.imshow(test_images[0][i],cmap='gray_r', vmin=0, vmax=255)
plt.figure()
